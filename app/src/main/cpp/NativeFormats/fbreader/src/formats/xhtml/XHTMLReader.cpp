...
void XHTMLTagStyleAction::doAtStart(XHTMLReader &reader, const char **xmlattributes) {
    ...
    if (reader.myReadState == XHTML_READ_NOTHING) {
        ...
        reader.myTableParser = new StyleSheetTableParser(reader.myPathPrefix,
                                                         reader.myStyleSheetTable, reader.myFontMap,
...
void XHTMLTagImageAction::doAtStart(XHTMLReader &reader, const char **xmlattributes) {
    ...
    bookReader(reader).addImage(imageName, new ZLFileImage(imageFile, EMPTY, 0, 0,
...
XHTMLTagSvgAction::XHTMLTagSvgAction(XHTMLSvgImageNamePredicate &predicate) : myPredicate(
        predicate) {
}
...
XHTMLSvgImageNamePredicate::XHTMLSvgImageNamePredicate() : ZLXMLReader::FullNamePredicate(
        ZLXMLNamespace::XLink, "href"), myIsEnabled(false) {
}
    ...
    if (href != 0 && href[0] != '\0') {
        ...
        if (hyperlinkType == INTERNAL_HYPERLINK) {
            ...
            if (epubType == 0) {
                ...
                static const ZLXMLReader::IgnoreCaseNamePredicate epubTypePredicate(
                        "epubu0003atype");
...
XHTMLTagParagraphWithControlAction::XHTMLTagParagraphWithControlAction(FBTextKind control)
        : myControl(control) {
}
...
XHTMLTagAction *XHTMLReader::addAction(const std::string &ns, const std::string &name,
                                       XHTMLTagAction *action) {
...
XHTMLTagAction *XHTMLReader::getAction(const std::string &tag) {
    ...
    for (std::map<shared_ptr<FullNamePredicate>, XHTMLTagAction *>::const_iterator it = ourNsTagActions.begin();
         it != ourNsTagActions.end(); ++it) {
...
XHTMLReader::XHTMLReader(BookReader &modelReader, shared_ptr<EncryptionMap> map) : myModelReader(
...
void XHTMLReader::applyTagStyles(const std::string &tag, const std::string &aClass) {
    ...
    for (std::vector<std::pair<CSSSelector, shared_ptr<ZLTextStyleEntry> > >::const_iterator it = controls.begin();
         it != controls.end(); ++it) {
...
void XHTMLReader::addTextStyleEntry(const ZLTextStyleEntry &entry, unsigned char depth) {
    ...
    for (std::vector<std::string>::const_iterator it = families.begin();
        ...
        if (!fontEntry.isNull()) {
            ...
            if (realFamily != *it) {
                ZLLogger::Instance().println("FONT",
                                             "Entry for " + *it + " stored as " + realFamily);
...
void XHTMLReader::startElementHandler(const char *tag, const char **attributes) {
    ...
    for (std::vector<std::string>::const_iterator it = classesList.begin();
         it != classesList.end(); ++it) {
...
void XHTMLReader::endElementHandler(const char *tag) {
    ...
    for (std::vector<shared_ptr<ZLTextStyleEntry> >::const_iterator jt = entries.begin();
         jt != entries.end(); ++jt) {
...
void XHTMLReader::beginParagraph(bool restarted) {
    ...
    for (std::vector<shared_ptr<TagData> >::const_iterator it = myTagDataStack.begin();
         it != myTagDataStack.end(); ++it) {
        ...
        for (std::vector<shared_ptr<ZLTextStyleEntry> >::const_iterator jt = entries.begin();
             jt != entries.end(); ++jt) {
            shared_ptr<ZLTextStyleEntry> entry = inheritedOnly ? (*jt)->inherited()
                                                               : (*jt)->start();
...
XHTMLReader::TagData::TagData() : PageBreakAfter(B3_UNDEFINED),
                                  DisplayCode(ZLTextStyleEntry::DC_INLINE) {
}
